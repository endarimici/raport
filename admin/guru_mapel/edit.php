...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Cek apakah mapping sudah ada (kecuali mapping yang sedang diedit)
    ...
    if (mysqli_num_rows($checkResult) > 0) {
        ...
        if (mysqli_query($conn, $query)) {
            ...
            $error = "Gagal mengupdate mapping: " . mysqli_error($conn);
        }
    }
}
?>
...
<head>
    ...
    <title>Edit Mapping Guru-Mapel - Aplikasi Raport SMK</title>
...
<body>
    <div class="dashboard">
        ...
        <div class="main-content">
            <div class="topbar">
                <h2>Edit Mapping Guru & Mata Pelajaran</h2>
                <div class="user-info">
                        ...
                        <div class="form-group">
                            ...
                            <select id="id_user" name="id_user" class="form-control" required>
                                ...
                                <?php while ($guru = mysqli_fetch_assoc($resultGuru)): ?>
                                    <option value="<?php echo $guru['id_user']; ?>" 
                                        <?php echo (isset($_POST['id_user']) ? $_POST['id_user'] : $data['id_user']) == $guru['id_user'] ? 'selected' : ''; ?>>
                            ...
                            <select id="id_mapel" name="id_mapel" class="form-control" required>
                                ...
                                <?php while ($mapel = mysqli_fetch_assoc($resultMapel)): ?>
                                    <option value="<?php echo $mapel['id_mapel']; ?>" 
                                        <?php echo (isset($_POST['id_mapel']) ? $_POST['id_mapel'] : $data['id_mapel']) == $mapel['id_mapel'] ? 'selected' : ''; ?>>
                            ...
                            <select id="id_rombel" name="id_rombel" class="form-control" required>
                                ...
                                <?php while ($rombel = mysqli_fetch_assoc($resultRombel)): ?>
                                    <option value="<?php echo $rombel['id_rombel']; ?>" 
                                        <?php echo (isset($_POST['id_rombel']) ? $_POST['id_rombel'] : $data['id_rombel']) == $rombel['id_rombel'] ? 'selected' : ''; ?>>
                            ...
                            <select id="id_semester" name="id_semester" class="form-control" required>
                                ...
                                <?php while ($semester = mysqli_fetch_assoc($resultSemester)): ?>
                                    <option value="<?php echo $semester['id_semester']; ?>" 
                                        <?php echo (isset($_POST['id_semester']) ? $_POST['id_semester'] : $data['id_semester']) == $semester['id_semester'] ? 'selected' : ''; ?>>
